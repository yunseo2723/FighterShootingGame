...
EnemyImage = ['Image/Enemy01.png', 'Image/Enemy02.png', 'Image/Enemy03.png', 'Image/Enemy04.png', 'Image/Enemy05.png', 'Image/Enemy06.png', 'Image/Enemy07.png', 'Image/Enemy08.png', 'Image/Enemy09.png']
explosionSound = ['music/explosionsound.mp3', 'music/explosionsound02.mp3']
highCount = 0
...
def initGame():
    global gamePad, clock, background, fighter, fighterWidth, fighterHeight, missile, explosion, missileSound, gameOverSound, item, itemWidth, itemHeight, Enemy, EnemyHeight, EnemyWidth, destroySound, itemSound
    pygame.init()
    ...
    item = pygame.image.load('Image/item.png')       # 아이템 그림
    item = pygame.transform.scale(item, (30, 30))  # 적기 크기 조정
    itemWidth = item.get_rect().size[0]
    itemHeight = item.get_rect().size[1]
    pygame.mixer.music.load('music/music.mp3')                 # 배경 음악
    pygame.mixer.music.play(0)                           # 배경 음악 재생
    missileSound = pygame.mixer.Sound('music/missile.mp3')     # 미사일 사운드
    itemSound = pygame.mixer.Sound('music/itemGet.mp3')     # 아이템 먹는 사운드
    gameOverSound = pygame.mixer.Sound('music/gameover.mp3')   # 게임 오버 사운드
...
def runGame():
    global gamePad, clock, background, fighter, fighterWidth, fighterHeight, missile, explosion, score, missileSound, highCount,shotCount, Enemy, EnemyHeight, EnemyWidth, destroySound
    ...
    onGame = False
    collision = False
    collision_sound_duration = 1.0  # 충돌 사운드 재생 시간 (초)
    collision_sound_start_time = 0.0  # 충돌 사운드 재생 시작 시간
    ...
    while not onGame:
        ...
        # 충돌 상태를 검사하고 미사일 발사 동작을 수행하는 부분
                    ...
                    if collision:
                        ...
                        # 충돌 상태가 아니면 기존과 동일하게 1발씩 미사일 발사
                ...
                if ((itemY + itemHeight) > y):
                    if (itemX > x and itemX < x + fighterWidth) or (itemX + itemWidth > x and itemX + itemWidth < x + fighterWidth):
                        if not collision_sound_playing:  # 충돌 사운드가 재생 중이 아닌 경우에만 재생
                            itemSound.play()  # 적기 폭발 사운드 재생
                            collision_sound_start_time = time.time()  # 충돌 사운드 재생 시작 시간 기록
                            collision_sound_playing = True  # 충돌 사운드 재생 중 상태로 설정
                        collision = True  # 충돌 상태로 설정
                        itemY = -75
                    else:
                        #collision = False  # 충돌 상태를 해제하여 1발씩 미사일 발사
                        collision_sound_playing = False  # 충돌 사운드 재생 중이 아님을 설정


        if collision_sound_playing and time.time() - collision_sound_start_time >= collision_sound_duration:
            itemSound.stop()
        ...
        if len(missileXY) != 0:
            for i, bxy in enumerate(missileXY):  # 미사일 요소에 대해 반복함
                bxy[1] -= 6   # 총알의 y좌표 -6 (위로 이동)
                missileXY[i][1] = bxy[1]

                # 미사일이 적기를 맞추었을 경우
                if bxy[1] < EnemyY:
                    if bxy[0]+EnemyWidth > EnemyX and bxy[0]-EnemyWidth < EnemyX + EnemyWidth:
...
def writeMessage(message):
    ...
    # 재시도 버튼 그리기
    restartButton = pygame.image.load('Image/Retry.png')  # 재시도 버튼 이미지 로드
    ...
    # 종료 버튼 그리기
    exitButton = pygame.image.load('Image/Exit.png')  # 종료 버튼 이미지 로드
